...
```javascript
pesa(200).add(22).currency('USD').sub(33).rate('INR', 0.013).mul(2, 'INR').to('INR', 75).round(2);
// 16585.65
```
...
Example:
```javascript
pesa(200.99).split([33, 66, 1]).map(m => m.float);
// [66.327, 132.653, 2.01]
...
```javascript
totalValue.to('USD', 59379.30).round(30);
...
```javascript
0.000000031032882086386885 * 101_234_318 * 59379.30;
// 186545.57265530445
```
